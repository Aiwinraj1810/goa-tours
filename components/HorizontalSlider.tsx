...
export default function HorizontalSlider({ className = "" }: HorizontalSliderProps) {
  ...
  const cards = spots.map((spot, index) => (
    <article
      key={spot.title}
      className="group relative flex min-h-[34rem] w-[min(88vw,24rem)] flex-none flex-col overflow-hidden rounded-[2rem] border border-white/50 bg-white/85 shadow-[0_20px_60px_rgba(8,122,187,0.12)] backdrop-blur md:w-[28rem] lg:w-[32rem]"
    >
  ...
  return (
    <>
      ...
      <div className="lg:hidden overflow-hidden px-4 py-6" ref={emblaRef}>
      ...
      <section ref={sectionRef} className={`relative hidden lg:block ${className}`}>
        <div className="overflow-visible">
          <div
            ref={trackRef}
            className="mx-auto flex w-max gap-6 px-10 py-6 lg:h-screen lg:items-center"
          >
            {cards}
          </div>
        </div>
